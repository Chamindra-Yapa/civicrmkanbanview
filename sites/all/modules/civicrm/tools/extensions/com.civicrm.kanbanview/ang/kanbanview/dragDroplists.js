 ...
 *
 * License: MIT
 * Slightly Modified by Chamindra to accomodate the case view of civicase knaban view
 */
  ...
  .directive('dndList', ['$parse', '$timeout', 'dndDropEffectWorkaround', 'dndDragTypeWorkaround',
    ...
    return function(scope, element, attr) {
      ...
      element.on('drop', function(event) {
        ...
        try {// Modified by Chamindra to get case status count
		    transferredObject = JSON.parse(data);
		  var pevStatusId=transferredObject.status_id;
		  	  
		  angular.forEach(scope.caseStatusCount,function(cc)  {
			 if (cc.key==attr.value)  {
				 cc.count++;
			 }	
			if (cc.key==pevStatusId)  {
				 cc.count--;
			 }				 
		  });
		  transferredObject.status_id=attr.value;
		  
		  scope.casesList.push(transferredObject);
		  
		  var caseObj=angular.copy(transferredObject);
		  var caseobjAray=Array();
		  caseobjAray.push(caseObj);
				  
		   CRM.api3('Case', 'replace', { //update with latest case status
						'id': caseObj.id,
						'values': caseobjAray
			});
		// Modified by Chamindra to get case status count and update status
        } catch(e) {
          return stopDragover();
        }
